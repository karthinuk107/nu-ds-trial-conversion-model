SELECT
  *,
  DATEDIFF(recent_activity_date,subscriptions_contractStartDate1) AS recency
FROM (
  SELECT
    cpn,
    subscriptions_contractStartDate1,
    DATE(contractEndDate) AS contractEndDate,
    COUNT(DISTINCT (activity_date)) AS active_days,
    MAX(activity_date) AS recent_activity_date,
    gender1,
    MAX(CASE
        WHEN dob <> '' THEN YEAR(CURRENT_TIMESTAMP()) -INTEGER(LEFT(dob, 4)) END) AS age,
    NVL(total_duration, 0) AS total_duration,
